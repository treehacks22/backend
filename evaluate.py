...
def evaluate(outputs: Variable, labels: Variable) -> float:
    ...
    Y = labels.numpy()
    Yhat = np.argmax(outputs, axis=1) # converts the outputs class probabilities into predicted classes
    return float(np.sum(Yhat == Y)) # checks if the predicted class matches the label class
...
def batch_evaluate(
    ...
    for batch in dataloader:
        n += len(batch['image'])
        # run inference on the neural network with this batch of images
